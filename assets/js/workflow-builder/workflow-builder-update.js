...
$(document).ready(function() {
    ...
    $('.selectField').change(function(e) {
        ...
        var key = parseInt($(this).attr('name').match(/[0-9]+/));
...
function init() {
    ...
    //delay for a bit
    setTimeout(function() {
        //we only want to know if custom_workflow is on
        //we dont care whether theres still seats left
        confirm_addons_subscription('custom_workflow', '', 'workflow-builder');
    }, 510);
}

function addCondition(condition_index) {
    var emptyFields = false;
    var applicationStatusSelected = null;
    ...
    $('.selectField').each(function(key, value) {
        ...
        if ($(this).val() == 'application_status') {
            applicationStatusSelected = $(this).closest('.conditionWrapper').find('.textValue').val();
        }
    });
    ...
    $.ajax({
        ...
        success: function(jObj) {
            ...
            $('.selectField').change(function() {
                ...
                if ($(this).val() == 'application_status_tag') {
                    $(this).closest('.conditionWrapper').find(".selectSubcondition option[value='OR']").remove();
                }
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        field: $(this).val(),
                        selectedFields: selectedFields,
                        applicationStatusSelected: applicationStatusSelected
                    },
                    success: function(jObj) {
                        container.html(jObj.html);
